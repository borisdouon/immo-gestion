...
export function SectionCardsTenant() {
  const totalTenants = mockTenants.length
  const activeTenants = mockTenants.filter(t => t.status === 'Actif').length
  const pendingTenants = mockTenants.filter(t => t.status === 'En Attente').length
  ...
  return (
    <div className="grid grid-cols-1 gap-4 px-4 sm:grid-cols-2 lg:px-6 xl:grid-cols-4">
      <KpiCard
        ...
        icon={<IconUsers className="size-6 text-orange-600 dark:text-orange-400" />}
      ...
      <KpiCard
        ...
        icon={<IconUserCheck className="size-6 text-emerald-600 dark:text-emerald-400" />}
      ...
      <KpiCard
        ...
        value="92%"
        subtitle="Ce mois"
        icon={<IconReceipt className="size-6 text-blue-600 dark:text-blue-400" />}
